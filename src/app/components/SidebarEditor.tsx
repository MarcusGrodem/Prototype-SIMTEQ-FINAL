...
export function SidebarEditor({ config, view, onToggle, onMoveUp, onMoveDown, onReset }: Props) {
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      {/* Visible order */}
      <div className="mb-1">
        <p className="text-[10px] text-gray-400 mb-1.5">Active order</p>
        <div className="space-y-0.5">
          {visibleItems.map((item) => {
            ...
            return (
              <div key={item.key} className="flex items-center gap-1.5 px-2 py-1.5 bg-white rounded border border-gray-200">
                ...
                <span className="text-xs text-gray-800 flex-1 truncate">{item.page.name}</span>
                <div className="flex gap-0 shrink-0">
                  ...
                  <button
                    ...
                    className="p-0.5 text-blue-400 hover:text-gray-400 transition-colors"
                    title="Hide"
                  >
          ...
          {visibleItems.length === 0 && (
            <p className="text-[10px] text-gray-400 text-center py-2 border border-dashed rounded">Nothing visible</p>
          )}
        </div>
      </div>
      ...
      <div className="border-t border-gray-100 pt-2 space-y-1">
        <p className="text-[10px] text-gray-400 mb-1.5">Hidden pages</p>
        {hiddenItems.map(item => {
          const Icon = item.page.icon
          return (
            <div key={item.key} className="flex items-center gap-1.5 px-2 py-1.5 bg-gray-50 rounded border border-dashed border-gray-200 opacity-70 hover:opacity-100 transition-opacity">
              <Icon className="w-3.5 h-3.5 text-gray-400 shrink-0" />
              <span className="text-xs text-gray-500 flex-1 truncate">{item.page.name}</span>
              <button
                onClick={() => onToggle(item.key)}
        ...
        {hiddenItems.length === 0 && (
          <p className="text-[10px] text-gray-400 text-center py-1">All pages are visible</p>
        )}
      </div>
    </div>
  )
